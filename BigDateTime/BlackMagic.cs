...
internal static class BlackMagic {
    ...
    public static int DayOfYear(BigInteger TotalSeconds) {
        // Alternative:
        // (BigInteger Year, int Month, int Day) = Date(TotalSeconds);
        // return BigDateTimeExtensions.DayOfYear(Year, Month, Day);

        return 1 + (int)((((uint)(((TotalSeconds / (SecondsInHour * 6)) | 3) % DaysIn400Years)) | 3) * EafMultiplier / EafDivider);
    }
    public static (BigInteger Year, int Month, int Day) Date(BigInteger TotalSeconds) {
